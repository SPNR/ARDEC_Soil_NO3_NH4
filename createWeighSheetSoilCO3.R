...
createWeighSheet <- function() {
  ...
  # the weigh sheet
  path <- 'W:/Laboratory/Data Handling/data tables/templates/'
  fileExt <- '.xlsx'
  defaultTemplateFile <- paste(path, 'ARDEC_Soil_CO3_Template', fileExt, sep = '')
  ...
  #Read template file
  library(xlsx)
  #Template worksheet 2 contains the classes for each column used on worksheet 1
  ...
  dataFileName <- paste('ARDEC_Soil_CO3_', as.character(year), sep = '')
  ...
  if(st == 'DMP') {
    ...
    # Otherwise subset the non-DMP weighSheet based on whether pltsfx is defined
  } else {
    if(is.na(pltsfx)) {
      weighSheet <- subset(templateSheet, study == st)
    } else {
...
#   }
  ...
  addTitle(weighSheetWS, rowIndex = 2, title = 'Soil Carbonate',
