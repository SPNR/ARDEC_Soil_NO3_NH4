...
readSoilWeights <- function() {
  ...
  if(length(dataFileName) == 0) fatalError('No data file selected')
  cat('\n\n...reading data file...')
  ...
  if(length(dupIndices) > 1) {
    fatalError(cat('Duplicate lab number(s) present in soil data file:',
  ...
  if(is.na(folder)) fatalError('No folder selected')
  ...
  # If duplicates are present then throw a fatal error
  if(length(dupIndices) > 1) {
    fatalError(cat('Duplicate lab number(s) present in soil weight files:',
